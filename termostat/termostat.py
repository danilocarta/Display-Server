...
limit = 25                              #define the temperature limit
...
def loop():
    ...
    while(True):
        ...
        if counts < 1:
            alertor("RaspiServer:\nTermostato attivo! Temperatura iniziale rilevata: "+("{0:0.1f}").format(temperature))
        counts=counts+1
        if temperature > 0:
...
def alertor(message):
    buzzState = False
    url = "https://toidigitalagency.it/ajax/alert/8/" + urllib.parse.urlencoder(message)
    try:
